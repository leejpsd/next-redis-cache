...
type ClientState =
  ...
  | {
      ...
      metrics: {
        ...
        payloadGeneratedBy: string;
        payloadFetchedAt: number;
      };
    };
...
function isRandomUserPayload(value: unknown): value is RandomUserPayload {
  if (!value || typeof value !== "object") return false;

  const candidate = value as RandomUserPayload;
  return (
    Array.isArray(candidate.results) &&
    typeof candidate.fetchedAt === "number" &&
    (candidate.source === "origin" || candidate.source === "fallback") &&
    typeof candidate.generatedBy?.instanceId === "string"
  );
}
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const payload = isRandomUserPayload(rawPayload)
          ? rawPayload
          : mapOriginPayload(rawPayload as RandomUserOriginResponse, mode);
        ...
        if (!cancelled) {
          setState({
            ...
            metrics: {
              ...
              payloadGeneratedBy: payload.generatedBy.instanceId,
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-stone-300/80 bg-white/80 px-4 py-3">
          ...
          <p className="mt-2 break-all font-mono text-sm text-stone-800">
            {state.metrics.payloadGeneratedBy}
          </p>
        </div>
        <div className="rounded-2xl border border-stone-300/80 bg-white/80 px-4 py-3">
          <p className="text-[11px] uppercase tracking-[0.14em] text-stone-500">
          ...
          <p className="mt-2 text-sm font-medium text-stone-800">
            {new Date(state.metrics.payloadFetchedAt).toLocaleString("ko-KR")}
          </p>
        </div>
      </div>
